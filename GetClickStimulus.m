function [LeftClickTrain, RightClickTrain] = GetClickStimulus(iTrial, Duration, SamplingRate, ClickLength, SoundLevel, Mode, BlockBias, TotalClicks)
    % TotalClicks is the new parameter to specify the total number of clicks
    global TaskParameters
    global BpodSystem

    if nargin < 7 || isempty(BlockBias)
    ...
    if nargin < 8
        TotalClicks = 100;  % Default value for TotalClicks
    end
    ...
    switch Mode
        case 'uniform'
            rr = rand(1, 1) * 0.6 + 0.2;
            LeftClickRate = ceil(rr * TotalClicks);
        ...
        case 'beta'
            ...
            LeftClickRate = round(BpodSystem.Data.Custom.TrialData.AuditoryOmega(iTrial) * TotalClicks);
            RightClickRate = TotalClicks - LeftClickRate;
